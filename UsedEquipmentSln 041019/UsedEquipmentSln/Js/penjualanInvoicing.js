$(document).ready(function() {
    ...
    $("#txt_amount_invoice , #txt_amount_factur , #txt_selling_price, #txt_incl_vat_id, #txt_incl_vat_us, #txt_excl_vat_id, #txt_excl_vat_us").keydown(function (event) {
        ...
        buttons: {
            'confirm': {
                ...
                action: function() {
                    ...
                    if ($('#txt_incl_vat_id').val().length != 0) {
                        incl_vat_id = parseFloat($('#txt_incl_vat_id').val());
                    }

                    if ($('#txt_incl_vat_us').val().length != 0) {
                        incl_vat_us = parseFloat($('#txt_incl_vat_us').val());
                    }
                    
                    if ($('#txt_excl_vat_id').val().length != 0) {
                        excl_vat_id = parseFloat($('#txt_excl_vat_id').val());
                    }

                    if ($('#txt_excl_vat_us').val().length != 0) {
                        excl_vat_us = parseFloat($('#txt_excl_vat_us').val());
                    }
                    ...
                    if ($('#txt_cus_id').val().length == 0) {
                        ...
                        var values_part = {
                            ...
                            MEDIATOR: (($('#txt_mediator').val().length == 0) ? null : $('#txt_mediator').val()),
                            ...
                            CONDITIONAL_DETAIL: (($('#txt_conditional_detail').val().length == 0) ? null : $('#txt_conditional_detail').val()),
                            EXPORT_DOMESTIK: (($('#txt_exdom').val().length == 0) ? null : $('#txt_exdom').val()),
                            SALES_TERM: (($('#txt_sales_term').val().length == 0) ? null : $('#txt_sales_term').val()),
                            DELIVERY_TERM: (($('#txt_deliv_term').val().length == 0) ? null : $('#txt_deliv_term').val()), 
                            DELIVERY_LOCATION: (($('#txt_deliv_locations').val().length == 0) ? null : $('#txt_deliv_locations').val()),
                            PRICE_INCL_VAT_RP : incl_vat_id,
                            PRICE_INCL_VAT_US : incl_vat_us,
                            PRICE_EXCL_VAT_RP : excl_vat_id, 
                            PRICE_EXCL_VAT_US: excl_vat_us,
...
function loadgrid() {
    ...
    var grid = $("#grid_ivoicing").kendoGrid({
        toolbar: [{ template: '<button id="btn_ex" type="button" class="k-button" onclick= GridToExcel() >Export to Excel</button>' }],
        //excel: {
...
function GridToExcel() {
    window.location.href = $('#urlPath').val() + "/PenjualanInvoicing/ExportToExcel";
}